...
The SDK supports various interactive features to create engaging user experiences.
:::
...
class CameraControlView: UIView {
    ...
    private func setupUI() {
        ...
        zoomOutButton = createButton(title: "−", action: #selector(zoomOutTapped))
...
- (void)setupUI {
    ...
    _zoomOutButton = [self createButtonWithTitle:@"−" action:@selector(zoomOutTapped)];
    ...
    func mapView(_ mapView: BCMapView, didSelectLocation location: BCLocation) {
        if isMultiSelectionEnabled {
            ...
            if !selectedLocations.contains(where: { $0.id == location.id }) {
                ...
                print("✅ Added to selection: \(location.name ?? "Unknown")")
            } else {
                // Remove from selection if already selected
                selectedLocations.removeAll { $0.id == location.id }
                print("❌ Removed from selection: \(location.name ?? "Unknown")")
            }
        } else {
            // Single selection mode
            selectedLocations = [location]
            print("📍 Single selection: \(location.name ?? "Unknown")")
        }
...
- (void)mapView:(BCMapView *)mapView didSelectLocation:(BCLocation *)location {
    if (self.isMultiSelectionEnabled) {
        ...
        if (!alreadySelected) {
            ...
            NSLog(@"✅ Added to selection: %@", location.name ?: @"Unknown");
        } else {
            ...
            NSLog(@"❌ Removed from selection: %@", location.name ?: @"Unknown");
        }
    } else {
        // Single selection mode
        [self.selectedLocations removeAllObjects];
        [self.selectedLocations addObject:location];
        NSLog(@"📍 Single selection: %@", location.name ?: @"Unknown");
    }
